...
static void wifi_sync_mbox_cb(lv_task_t *t)
{
    ...
    bool ret = false;
    static int retry = 0;
    //configTzTime(RTC_TIME_ZONE, "au.pool.ntp.org"); //dw added au prefix

     // dw 
     // Init and get the time
     long gmtOffset_sec, daylightOffset_sec;
     gmtOffset_sec = -9 * 3600;    //dw EST is 10 hours early
     daylightOffset_sec = 3600;      //dw one hour is 3600 secs
     configTime(gmtOffset_sec, daylightOffset_sec, "au.pool.nt.org");
    ...
    while (1) {
        ...
        ret = getNTPtime(20);  // wait up to 10sec to sync //dw use stuff from example
        if (!ret) {
            Serial.printf("get ntp fail,retry : %d \n", retry++);
        } else {
            //dw 
            Serial.println(&timeinfo, "LocalTime : %A, %B %d %Y %H:%M:%S");    // DW Day in full, Month in full, Day (numeric), Hours, Mins, Secs //
            //dw
            //! del preload
            delete pl;
            pl = nullptr;
            ...
            char format[256];
            snprintf(format, sizeof(format), "Time acquired is:%d-%d-%d/%d:%d:%d, Do you want to synchronize?", timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
...
void wifi_sw_event_cb(uint8_t index, bool en)
{
    ...
    switch (index) {
    case 0: //dw switches CPU frequency
        if (en) {
    ...
    case 1: // dw scans WiFi networks
      if (en)
      {
        ...
        break;
    case 2:   // dw does NTP request and asks if sync should happen
      if (en)
      {
        if (!WiFi.isConnected()) {
            //TODO pop-up window
...
int createWiFiSwitches (int idx)
{
  ...
  log_d ("idx=%d", idx);
    Switch::switch_cfg_t cfg[4] = {{"Switch", wifi_sw_event_cb}, {"Scan", wifi_sw_event_cb}, {"NTP Sync", wifi_sw_event_cb},{"Def WiFi",wifi_sw_event_cb} };  //dw added connect default WiFi, set ssid and password in gui.h
    sw = new Switch;
    sw->create(cfg, 4,
	      nullptr,
        parentObj);
