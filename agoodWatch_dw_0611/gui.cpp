...
const char* NTP_SERVER = "au.pool.ntp.org";
const char* TZ_INFO =   "AEST-10AEDT,M10.1.0,M4.1.0/3";  // Sydney-Australia
//dw
#ifdef DEBUG_EVENTS
#define DEBUG_EVENTS
/*
  ...
  //dw
    ...
    while (1) {
        ...
        if (!ret) {
            ...
            char format[256];
            snprintf(format, sizeof(format), "Time acquired is: %d-%d-%d/%d:%d:%d, Want to synchronize?", timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
